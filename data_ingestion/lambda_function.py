...
# This value may need to be adjusted depending on the volume of the data.
...
def load_last_run_timestamp():
    ...
    try:
        ...
        if e.response['Error']['Code'] == 'NoSuchKey':
            logger.info(f"Last run timestamp object '{S3_LAST_RUN_KEY}' not found in S3 bucket. Performing a full historical pull.")
            return None
        else:
            logger.info(f"Error loading timestamp from S3: {e}")
            raise
...
def save_current_timestamp():
    ...
    try:
        ...
        logger.info(f"Saved new timestamp for next run to S3: {timestamp_str}")
    except Exception as e:
        logger.info(f"Error saving timestamp to S3: {e}")
        raise
...
def make_api_request(url, headers, params=None, max_retries=5, backoff_factor=1.5):
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.info(f"Received status code {response.status_code}. Retrying in {backoff_factor**attempt} seconds...")
    ...
    logger.info(f"Failed to fetch data from {url} after {max_retries} attempts.")
    return None
...
def fetch_media_show(media_id, headers):
    ...
    if response and response.status_code == 200:
        ...
        logger.info(f"Error fetching stats for media ID {media_id}: {response.status_code if response else 'No Response'} - {response.text if response else ''}")
        return None
...
def fetch_incremental_data(url, last_run_timestamp, headers, page_limit=None):
    ...
    logger.info(f"Starting incremental pull from {last_run_timestamp or 'beginning of time'}.")

    while True:
        if page_limit and page > page_limit:
            logger.info(f"--- Reached page limit of {page_limit}. Stopping pull. ---")
            break
        ...
        if not response or response.status_code != 200 or not response.json():
            logger.info("--- Reached end of data or encountered an error. ---")
            break
        ...
        for record in response.json():
            ...
            if last_run_timestamp and record_timestamp <= last_run_timestamp:
                ...
                logger.info(f"--- Found old record from {record_timestamp}. Stopping pull. ---")
                break
...
def upload_data_to_s3(data, file_id, timestamp, prefix):
    ...
    if not data:
        logger.info(f"No new data to upload for '{file_id}'.")
        return
    ...
    try:
        ...
        logger.info(f"Successfully uploaded {len(data)} records to S3 at s3://{S3_BUCKET_NAME}/{file_key}")
    except Exception as e:
        logger.info(f"Error uploading data to S3: {e}")
        raise
...
def upload_media_stats_to_s3(data, media_id, prefix="media-stats"):
    ...
    if not data:
        logger.info(f"No media stats to upload for media ID {media_id}.")
        return
    ...
    try:
        ...
        logger.info(f"Successfully uploaded media stats to S3 at s3://{S3_BUCKET_NAME}/{file_key}")
    except Exception as e:
        logger.info(f"Error uploading media stats to S3: {e}")
        raise
...
def main():
    ...
    try:
        ...
        logger.info(f"Failed to retrieve API token. Exiting.")
    ...
    if new_visitor_stats:
        ...
        logger.info("No new visitor data found.")
    ...
    for media_id in MEDIA_IDS:
        ...
        if new_event_stats:
            ...
            logger.info("No new events data found.")
