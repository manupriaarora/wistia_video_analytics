...
def fetch_media_stats(media_id, headers):
    ...
    if response.status_code == 200:
        ...
        print(f"Error fetching stats for media ID {media_id}: {response.status_code} - {response.text}")
        return None

def fetch_incremental_visitors(media_id, last_run_timestamp, headers):
    """
    ...
    while True:
        ...
        all_new_visitors.extend(new_records_on_page)
    ...
    return all_new_visitors

def upload_data_to_s3(data, media_id, timestamp):
    ...
    file_key = f"wistia-pipeline/visitor-stats/{media_id}-{timestamp.strftime('%Y-%m-%d-%H-%M-%S')}.json"
...
def upload_media_stats_to_s3(data, media_id):
    ...
    file_key = f"wistia-pipeline/media-stats/{media_id}.json"
...
def main():
    ...
    for media_id in MEDIA_IDS:
        ...
        new_visitor_stats = fetch_incremental_visitors(media_id, last_run_timestamp, headers)

        if new_visitor_stats:
            # 4. Upload the new data to S3
            upload_data_to_s3(new_visitor_stats, media_id, current_run_timestamp)
        else:
